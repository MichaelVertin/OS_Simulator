    ...
    switch( controlCode )
       {
        ...
        case STOP_TIMER:
           ...
           break;
/*
        // integer form of processTime
        case GET_TIME:
           gettimeofday( &intermediateData, NULL );

           // extract intermediate time from current intermediate time of day
           interSec = intermediateData.tv_sec;
           interUSec = intermediateData.tv_usec;

           // get difference in time
           interSec -= startSec;
           interUSec -= startUSec;

           // check for usec negative (start's usec is higher)
           if( interUSec < 0 )
           {
               // move a second from sec into usec
               interSec--;
               interUsec += 100000;
           }

           timeToString( interSec, interUSec, timeStr );
*/
       }
...
void currentTimeToString( char *timeStr )
{
    ...
    // store current time of day into currentTime
    gettimeofday( &currentTime, NULL );

    // store currentTime into timeStr
    timeToString( currentTime.tv_sec, currentTime.tv_usec, timeStr );
}


void *runTimerThread( void *delayVoidPtr )
{
    int *delayPtr = (int *)delayVoidPtr;
    int delay = *delayPtr;
